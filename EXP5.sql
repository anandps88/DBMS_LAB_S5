...
WHERE ORDERDATE LIKE '%-JAN-%' AND PRONO IN (
    SELECT PRONO FROM PURCHASE 
    WHERE PRONO = 'P00002'
);
...
SELECT DISTINCT P1.CUSTNO AS CUST_1, P2.CUSTNO AS CUST_2 FROM PURCHASE P1, PURCHASE P2 WHERE (
    SELECT PRICE FROM PRODUCT 
    WHERE PRODUCT_NO=P2.PRONO 
)>(
    SELECT PRICE FROM PRODUCT 
    WHERE PRODUCT_NO=P1.PRONO
) 
AND P1.CUSTNO <> P2.CUSTNO;
...
SELECT DISTINCT C1.NAME AS CUST_1, C2.NAME AS CUST_2 FROM CUSTOMER C1,CUSTOMER C2,PURCHASE P1, PURCHASE P2 
WHERE P1.CUSTNO=C1.CUST_NO 
AND P2.CUSTNO=C1.CUST_NO 
AND EXISTS(
    SELECT 1 FROM PRODUCT R1, PRODUCT R2 
    WHERE P1.PRONO=R1.PRODUCT_NO 
    AND P1.PRONO=R2.PRODUCT_NO 
    AND R1.PRICE<1600 
    AND R2.PRICE<1600
) 
...
SELECT DISTINCT PRONO FROM PURCHASE P1 
WHERE EXISTS(
    SELECT 1 FROM PURCHASE P2 
    WHERE P1.PRONO=P2.PRONO 
    AND P1.CUSTNO <> P2.CUSTNO);
...
WHERE EXISTS(
    SELECT 0 FROM PURCHASE P 
    WHERE P.CUSTNO=C.CUST_NO 
    AND P.PRONO='P00002'
);
...
WHERE EXISTS(
    ...
    WHERE P.CUSTNO=C.CUST_NO 
    AND P.PRONO IN (
        SELECT PRONO FROM PURCHASE P1 
        WHERE P1.CUSTNO='C00004'
    )
);
