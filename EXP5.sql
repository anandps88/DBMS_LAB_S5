...
SELECT CUSTNO FROM PURCHASE WHERE PRONO IN (SELECT PRODUCT_NO FROM PRODUCT WHERE PRICE >1600);
...
SELECT NAME FROM CUSTOMER WHERE CUST_NO IN (SELECT CUSTNO FROM PURCHASE WHERE ORDERDATE LIKE '%-JAN-%' AND PRONO IN (SELECT PRODUCT_NO FROM PRODUCT WHERE PRICE > 1500));
...
SELECT T1.NAME FROM CUSTOMER T1 JOIN PURCHASE T2 ON T1.CUST_NO= T2.CUSTNO JOIN PRODUCT T3 ON T2.PRONO = T3.PRODUCT_NO WHERE T3.MANUFACTURER = 'WIPRO';
